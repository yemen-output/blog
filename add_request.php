...
try {
    $highestId = getHighestId($db, $json_file, 'requests');
    $newId = $highestId + 1;

    $stmt = $db->prepare("INSERT INTO requests (id, name, phone, service_id, details, created_at) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->execute([$newId, $name, $phone, $service_id, $details, $created_at]);

    http_response_code(200); // OK
    echo json_encode(['message' => 'Service request added successfully.', 'request_id' => $newId]);
} catch (PDOException $e) {
    http_response_code(500); // Internal Server Error
    echo json_encode(['error' => 'Error adding service request: ' . $e->getMessage()]);
}
?>